...
In Figure 2, the data appears to be noramlly distributed across the three variables, concetration, opitimism, and sexual activity which were all rated from 1 to 10 from the 120 patients.
...
where \(\chi^2\) is the chi-squared statistic for the contingency table, \(n\) is the total number of observations, \(k\) is the number of columns, and \(r\) is the number of rows in the table.
...
where \(H(T)\) is the entropy of the target variable \(T\), \(X\) is the feature of interest, \(T_v\) is the subset of \(T\) where \(X\) takes value \(v\), and \(|\cdot|\) denotes the number of observations.
