...
Figure 1 shows a conceptual graph illustrating the iterative process of XGBoost.
...
```
Using Cramer's V we are able to see that mood swings and suicidal thoughts have the highest correlation to the mental health diagnosis. 
...
ggplot(info_gain_sorted, aes(x = reorder(attributes, importance), y = importance)) +
  ...
  coord_flip() +
  labs(title = "Information Gain by Variable",
       x = "Variable",
       y = "Information Gain") +
  theme_minimal()
...
Using information gain on the transformed dataset, mood swings and optimism provide the highest information gain. 
